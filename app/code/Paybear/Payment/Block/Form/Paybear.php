...
class Paybear extends \Magento\Framework\View\Element\Template
{
    ...
    )
    {
        ...
        $this->_paybearPayment = $paybearPayment;
        $this->helper = $helper;
        $this->url = $url;
    ...
    public function getInitialData()
    {
        ...
        $total_paid_data = $this->_paybearPayment->getAlreadyPaid($order->getId(), $order_currency_code, $orderGrandTotal);

        $fiat_value = $orderGrandTotal;
        $currencies_url = $this->url->getUrl('paybear/payment/currencies', [
            'order' => $order->getId(),
        ...
        //$currencies = $this->_paybearPayment->getCurrenciesJson((float)$order->getGrandTotal(), $order->getId(), $order_currency_code, $token);
        $data['fiat_value'] = $fiat_value;
        $data['currencies'] = $currencies_url;
        //$data['currencies'] = $currencies;

        $data['status_url'] = $this->url->getUrl('paybear/payment/status', [
            'order' => $order->getId()
        ]);
    ...
    public function getOrderInfo($order, $currency_sign, $fiat_value, $total_paid)
    {
        ...
        $button_html = '<a href="#" class="button paybear_button" id="paybear-all">Pay with Crypto</a>';
        if ($status == 'complete' || $status == 'processing') {
            $payment_status = 'Paid';
            $button_html = '<a href="' . $this->url->getUrl('checkout/onepage/success') . '" class="button paybear_button" >Continue</a>';
        }

        if (($status == 'awaiting_confirmations') && ($fiat_value < $underpayment_fiat)) {
            $payment_status = 'Waiting for Confirmations';
            $button_html = '<a href=""  class="button paybear_button" >Refresh</a>';
        ...
        if ($this->_paybearPayment->load($order->getId(), 'order_id')->getId()) {
            $token = $this->_paybearPayment->getToken();
            ...
            $address = $this->_paybearPayment->getAddress();
            if ($address) {
                $content .= '<tr class="woocommerce-table__line-item order_item">
                            <th>Payment address</th>
                            <td><a href="' . $this->_paybearPayment->getBlockExplorerUrl($token, $address) . '" target="_blank">' . $address . '</a></td>
                        </tr>';
            }
        }
