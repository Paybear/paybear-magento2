...
/**
 * @method \Paybear\Payment\Model\ResourceModel\Payment getResource()
 * @method \Paybear\Payment\Model\ResourceModel\Payment\Collection getCollection()
 */
class Payment extends \Magento\Framework\Model\AbstractModel implements \Paybear\Payment\Api\Data\PaymentInterface,
    \Magento\Framework\DataObject\IdentityInterface
{
    const CACHE_TAG = 'paybear_payment_payment';
    protected $_cacheTag = 'paybear_payment_payment';
    protected $_eventPrefix = 'paybear_payment_payment';
    ...
    {
        $this->_init('Paybear\Payment\Model\ResourceModel\Payment');
    }
    ...
    public function getRates($order_currency_code)
    {
        ...
        if (self::$_rates === null) {
            $url = sprintf('%s/v2/exchange/%s/rate', $this->apiDomain, $order_currency_code);
        ...
        if (empty($addressObject)) {
            ...
            if ($token_address_data->address) {
                ...
                if (empty($payment)) {
                    ...
                    $this->_setPaybearPayment($payment_data);
                } else {
                    $payment_data['address'] = $token_address_data->address;
                    $payment_data['invoice'] = $token_address_data->invoice;
                    $payment_data['updated_at'] = date('Y-m-d H:i:s');

                    $this->_updatePaybearPayment($payment_data, $payment);
                }
    ...
    {
        $callbackUrl = $this->url->getUrl('paybear/payment/callback', [
            'order' => $order_id
        ]);

        $url = sprintf('%s/v2/%s/payment/%s?token=%s', $this->apiDomain, $token, urlencode($callbackUrl), $this->apiKey);
    ...
    public function getCurrencies()
    {
        if (self::$_currencies === null) {
            $url = sprintf('%s/v2/currencies?token=%s', $this->apiDomain, $this->apiKey);
            $response = $this->request($url);
    ...
    protected function _setPaybearPayment($payment_data)
    {
        $this->setData($payment_data)->save();
    }

    protected function _updatePaybearPayment($payment_data, $payment)
    {
    ...
    {
        try {
            $paybear_payment = $this->load($order_id, 'order_id');
            if ($paybear_payment->getId()) {
                $token = $paybear_payment->getToken();

                $rate = round($orderGrandTotal / $paybear_payment->getAmount(), 8);
                $orderTimestamp = strtotime($paybear_payment->getPaidAt());
